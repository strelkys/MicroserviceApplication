 ...
 * @author Belonozhkin
 */
public class SweepIcon implements Icon {

    private static final Logger LOG = Logger.getLogger(SweepIcon.class);
    ...
    public SweepIcon(int hCnt, int vCnt, ColorScale colSc) {
        values = new double[hCnt][vCnt];
        this.colSc = colSc;
    }
    
    public SweepIcon(double[][] values, ColorScale colSc){
        this.values = values;
        this.colSc = colSc;
    }
    ...
    public void paintIcon(Component c, Graphics g, int x, int y) {
        ...
        for (int i = 0; i < values.length; i++) { //колличство столбцов 
            double xx = (double) c.getWidth() - (i + 1) * width; //c.getHeight() - 1 - (int) Math.round(hs * i); 
