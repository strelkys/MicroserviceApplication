...
public class ScaleIcon implements Icon {
    ...
    public void paintIcon(Component c, Graphics g, int x, int y) {
        ...
        double curX = (fullLenth - scaleLength);
        g.fillRect(0, 0, (int) (curX / mx), c.getHeight());
        double prtCoor = 0;
        int fh = g.getFontMetrics().getAscent() + g.getFontMetrics().getLeading();
        for (; curX < fullLenth; curX += mainStep) {
        ...
        for (curX = (fullLenth - scaleLength); curX < fullLenth; curX += smallStep) {
