 ...
 * @author SmorkalovAV
 */
public class StatProcMinMax extends StatProc {

    public int min; //Минимальное значение из поступивших.
    public int max; //Максимальное значение из поступивших.

    public StatProcMinMax() {
        super();
        reset();
    }

    public StatProcMinMax(float noise) {
        super(noise);
        reset();
    }

    public StatProcMinMax(float noise, int dVer) {
        super(noise, dVer);
        reset();
    }

    public StatProcMinMax(float noise, int dVer, int tmpsize) {
